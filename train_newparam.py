## Changes to match description in paper
...
######### ch added
save_prefix = "gcNHNH2RB" ## SET VARIABLE WHEN TRAINING DATASET CHANGES
...
parser.add_argument('-train_batch_size', help='Set the training batch size', default=16, type=int)  ## originally 25, paper used 16
parser.add_argument('-train_epoch', help='Set the training epoch', default=8005, type=int) ## paper used 8000 epochs
...
# --- Build optimizer --- #
G_optimizer = torch.optim.Adam(MyEnsembleNet.parameters(), lr=0.0001)  # Generator optimiser
scheduler_G = torch.optim.lr_scheduler.MultiStepLR(G_optimizer, milestones=[3000, 5000, 6000], gamma=0.5)  ## CH Change: Changed to match paper's stated experimental settings
D_optim = torch.optim.Adam(DNet.parameters(), lr=0.0001)  # Discriminator optimiser
scheduler_D = torch.optim.lr_scheduler.MultiStepLR(D_optim, milestones=[3000, 5000, 6000], gamma=0.5)  ## CH Change: Changed to match paper's stated experimental settings
# --- Load training data --- #
...
for epoch in range(train_epoch):
    ...
    for batch_idx, (hazy, clean) in enumerate(train_loader):
        ...
        total_loss = smooth_loss_l1 + (0.2 * msssim_loss_) + (0.001 * perceptual_loss) + (0.005 * adversarial_loss)   ## CH Change: Changed to match paper's stated function
        total_loss.backward()
        D_optim.step()
        G_optimizer.step()
    ...
    scheduler_G.step()  # CH change: moved optimizer.step after lr_scheduler.step as needed for pytorch>1.1.0
